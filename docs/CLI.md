...
- `-o`/`--output <path>` (M8.7, `run`/`send` only) — writes the raw response body to `<path>` byte-exact, with no lossy utf8/base64 round-trip through the `--json` envelope shape (which still prints to stdout independently, so `-o` and `--json` freely combine: body → file, envelope → stdout). `-o -` writes the raw body to stdout instead of a file (interleaves with the envelope under `--json`, since both then target stdout — pick one or the other in that mode). A relative path resolves against the process's current directory, never the workspace root. Parent directories are created as needed; the write itself is atomic. A write failure is `error.kind: "output-write-failed"`, exit **5** — the request itself already succeeded, so this is a post-execution I/O failure, not a transport one.
...
| `output-write-failed` | 5 | `-o/--output <path>` (M8.7): the request succeeded, but writing the raw response body to `path` failed (bad/permission-denied path, disk error) |
