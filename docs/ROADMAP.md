...
### Still deferred ⏳
- **`churl import` default behaviour → M8.2 (CLI & headless).** Today the CLI `import` **prints TOML** to stdout (`--out FILE` to persist); a user reasonably expects "import" to *create* an endpoint in the cwd workspace, as the TUI does. And piping non-curl content (`pbpaste | churl import` over a TOML dump) yields a confusing parse error. Both are `churl import`'s contract, so they're folded into the M8.2 CLI redesign (alongside `churl init`, headless `send`/`run`, `--help` refinement) rather than patched piecemeal — likely `import` creates by default, `--out FILE` for an explicit path, `--print` for the TOML dump, and a clear "not a curl command" error on non-curl stdin.
