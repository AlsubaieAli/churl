...
| Minified single-line body re-materialized every frame (perf cliff — the M7.7 formatter issue) + unsanitized ANSI/control chars + tab-width-0 | P1 | ✅ done (M7.7 — reformatter for the common case, horizontal-window slice bounds the rest; `sanitize_for_display` strips ANSI/controls + expands tabs at `TAB_WIDTH=4`) |
...
**Deliverables** (all done, 2026-07-11 — branch `fix/r1-durability`, behaviour-preserving in structure; the M7.11 module reorg is deliberately not pre-empted):
- **Reserved-name guards (D1) — DONE.** `create_endpoint`/`rename_endpoint`/`create_sequence`/`rename_sequence` fold a slug equal to a reserved *file* stem (`churl`/`folder`, i.e. the `MANIFEST_FILENAME`/`FOLDER_FILENAME` stems) into the "occupied" set `unique_endpoint_path` already avoids, so `churl` → `churl-2.toml` — disambiguates instead of silently overwriting the manifest/folder file. `create_collection`/`rename_collection` do the same for the reserved *dir* name (`sequences`, `SEQUENCES_DIRNAME`) via `collection_dir_name` (a collection named `churl`/`folder` is NOT a hazard — those are files). The display `name` field is always what the user typed; the disambiguated on-disk stem is surfaced in the TUI create/rename confirmation (`created X (as X-2 — name was reserved)`) — no silent no-op. `RESERVED_FILE_STEMS`/`RESERVED_DIR_NAMES` are kept in sync with the filename constants by a test.
- **Merge-comment preservation (D3) — DONE.** The `(ArrayOfTables, ArrayOfTables)` arm of `merge_items` now merges element-wise regardless of length (`merge_arrays_of_tables`): recurse into the overlapping prefix (survivors keep their decor), append genuinely new tables, truncate removed trailing ones — replacing the old wholesale `*old = new.clone()` that discarded all `# comments`/whitespace/order on surviving siblings when the array grew or shrank.
- **SQLite concurrency (D2) — DONE.** `HistoryStore::open` sets `PRAGMA busy_timeout=5000` (const `BUSY_TIMEOUT_MS`) then `PRAGMA journal_mode=WAL` and fails loud (`HistoryError::WalUnavailable`) if the FS silently fell back (file-backed DBs only; in-memory legitimately reports `memory`). `migrate()` runs the whole loop inside ONE `BEGIN IMMEDIATE` transaction, re-reads `user_version` under the write lock, applies only still-pending migrations, and commits once — all-or-nothing, so a concurrent process blocks then no-ops.
- **History pruning (D4b) — DONE.** `insert`/`insert_load_batch` prune-on-insert back to the newest `HISTORY_ROW_CAP`=10 000 rows per table via a cheap `DELETE … WHERE id <= max_id - CAP` (indexed range delete, no per-insert subquery). `recent()` reads are unaffected; `workspaces` is naturally bounded (one row per path).
- **Bounds — bounded channel (D4a) + highlight-cache eviction (D4c) — DONE.** The app channel is now bounded `mpsc::channel(1024)` (const `APP_CHANNEL_CAPACITY`); all four `AppMsg` senders are in spawned async tasks (`.send().await`) and the highlight worker is a dedicated OS thread (`blocking_send`) — none on the render/UI thread, so a full queue applies backpressure without stalling input. Highlight caches are evicted from the *previously active* buffer on every buffer switch (`set_active_buffer`), bounding total cached lines to (active-buffers × 64) rather than (all-buffers × 64). Open-buffer LRU capping was **not** implemented (see Deviations in the R1 report) — buffers are user-opened and bounded by intent; the unbounded growth vectors (channel, SQLite, highlight cache) are all closed.
- Tests: reserved-name collision + property-style guard + rename-into-reserved (persistence integration), array comment-survival on shrink/grow, WAL+busy_timeout assert + second-open no-op + concurrent connections, history/load_batch pruning, buffer-switch cache eviction. Baseline 848 → 865 green.
- Docs: DECISIONS ADRs (reserved-name disambiguation · WAL+busy_timeout+migration-lock · element-wise array merge · the three bounds + const values); ARCHITECTURE persistence/history/highlight rows updated; REGRESSION rows added. (The `interchange` module-table row + clipboard-section cleanup remain open follow-ups, not durability.)
...
**M7.4 drive-test follow-up (note #6) — extraction rule → persistent Session variable** (owner-designed, unnumbered; branch `feat/n6-session-vars`, 2026-07-10) — a step's extraction rule gains a per-rule target: **Run-only** (default, today's ephemeral behaviour) or **Session**. A Session-target rule's captured value is written into a process-lifetime, **in-memory** Session store (`App::session_vars`, keyed by canonical workspace root) that is **NEVER written to disk** (the owner's "as secure as possible" call — captured secrets stay in RAM, evaporate on exit). A new resolver scope `session` sits `extracted > session > cli > profile > collection > workspace` and is injected for BOTH sequence-run and standalone-send resolution, so a token captured by a `login` sequence resolves in a later standalone `{{token}}` send. Model: `SequenceStep.persist: Vec<String>` (the Session-target rule *names*, serialized as `persist = [...]`, backward-compatible + format-preserving — `extract` untouched). Editor: `p` in the Rules focus toggles a rule Run-only ⇄ Session with a dim ` →session` marker. Env editor: a read-only, masked `Session` scope group with a `c` clear action. Security asserted by `session_value_never_written_to_disk` (whole-tree scan + byte-identical TOMLs) and masking in the env editor. See the 2026-07-10 note-#6 ADR in DECISIONS.
...
- **Optional A→Z key-sort toggle** — ✅ **done (Stage A follow-up)**. On top of the pretty view, `s` (`Action::ToggleSortKeys`) toggles an **optional alphabetical sort** of every JSON object's keys, recursively (arrays keep element order). **Default is wire order** (unchanged); sorting is an explicit opt-in and is **ephemeral per response** (resets on the next response, nothing persisted). Only meaningful on a pretty JSON body — on a raw or non-JSON view it no-ops with a `sort: pretty JSON only` notice. Like `p`, toggling rebuilds `text`/`line_offsets`, bumps `generation`, resets folds and clears search. Copy (`y`) still returns the exact raw on-the-wire bytes regardless of sort/pretty state.
- **Help-overlay `/` quick-search** — ✅ **done (Stage B)**: `/` inside the `?` help pane is **highlight-and-jump** — it opens an input, typing locates matches over the help lines' plain text, `n`/`N` cycle (wrap), Esc cancels (search cleared, overlay stays open), Enter commits (matches retained, input closed). **Rows are never hidden** — matches are highlighted in place (current reversed, others dim+underlined, same styling as the response body search) and the view jumps to the current match; the title shows the live query + `k/N` counter. Reuses the response body-search engine via a **shared smart-case matcher** (`response::smart_case_matches`) + shared span painter (`response::build_search_spans`) so the two searches cannot diverge. Keeps the live-KeyMap-driven help from M6.7.
- Tests (Stage A): pretty-by-default for minified JSON, pretty↔raw toggle round-trips byte-exact, malformed JSON stays raw, fold + wrap over pretty text, copy stays raw. Stage B: search locates a known label with correct byte ranges, `n`/`N` cycle+wrap, Esc restores / Enter commits (App-level), smart-case parity via the shared matcher, rows-not-hidden (line count unchanged under an active search), plus the updated help-overlay snapshot (title carries the `/ search` hint).
- Also fold in (from the 07-09 sweep, later stages) — ✅ **done (close-out)**: **control-char / ANSI sanitize** + explicit tab-width in the viewer (`sanitize_for_display` in `components/response.rs`: strip CSI/OSC/lone-ESC → expand tabs to the next `TAB_WIDTH=4` stop → normalize `\r\n`/lone-`\r` → replace remaining C0/DEL/C1 controls with `·`; applied to the reformatted **displayed** text before `index_lines`, leaving `raw_text` untouched so copy stays byte-exact), and the **horizontal-window slice** for unwrapped long lines (an `h_scroll` char-column offset on `ResponseView`; unwrapped normal-line rows build only the `[h_scroll, h_scroll+width)` window instead of the whole line, bounding the minified/non-JSON perf cliff — reformat addresses the common JSON case, the window slice bounds the rest; `H`/`L` + Left/Right pan it, clamped to the widest visible line, no-op under wrap, with search-into-view panning a far-right match into the window). The f-jump action label was corrected to "jump to pane" (M7.10 stage B made it pane-only).

**M7.7 drive-test follow-up (note #8) — line-number gutter, default ON** (owner drive-test 2026-07-10, branch `feat/n8-line-numbers`) — the response viewer now shows a left-hand **line-number gutter**, **on by default**, toggled by `#` (`Action::ToggleLineNumbers`, remappable). Numbers are the **1-based logical lines of the displayed (post-reformat) `text`**; a wrapped line's continuation rows show a blank, aligned gutter; a fold-header shows its opener line's number; the gutter applies to the headers view too, and appears in the load-runner / sequence-runner responses via the shared `ResponseView`. The gutter width is stable (digits of the total displayed line count + 1 pad). **Width seam**: `render_done` subtracts the gutter width from `body_area.width` once and every downstream consumer (wrap split, `h_scroll` window, `viewport_width`, and thus the caller's cursor→logical mapping) uses that reduced body width — see the 2026-07-10 ADR in DECISIONS. `raw_text`/copy untouched (render-only). One `bool` of view state; default-on re-established per response (a new response builds a fresh view).

**M7.7 drive-test follow-up (note #4b) — fail loud on unresolved `{{var}}`** (owner drive-test finding #4b, branch `fix/unresolved-template-loud`). Before: an unresolved `{{var}}` (no scope resolves it) was left as a **literal** `{{var}}` in the substituted request and shipped — e.g. `…/get?echoed={{echoed}}`, whose `{`/`}` make an invalid URL and produce a cryptic reqwest `error sending request for url (…)`; the user never learns a variable was unresolved. Now every send path detects leftover placeholders **after** substitution and **refuses to send**, naming the variable(s): `unresolved variable(s): echoed, token — set them in a profile/env or via CLI`. Detection is a single core seam, `churl_core::template::unresolved_placeholders(req) -> Vec<String>`, that reuses the resolver's **own** delimiter scan (`parse_placeholder`), so it flags exactly what substitution would have replaced — a malformed/literal brace run (`{{ }}`, `{{a b}}`, an unclosed `{{`; churl has no other double-brace escape) is left verbatim by substitution and is **never** flagged. Wired at all three send sites: main-pane send (`App::send_request`, checked before the client gate), the load runner (`App::open_load_runner` resolves once at open time — refuses to open so the whole batch never fires), and sequence steps (`sequence::prepare_step` returns `SequenceError::Unresolved { names }`, so the step fails honoring the sequence's `on_error` halt/continue). Success path and resolver precedence unchanged — a fully-resolved request behaves identically. See the 2026-07-11 ADR in DECISIONS.md.

**Drive-test follow-up (note #4a) — honest, yankable failed-response row** (owner drive-test finding #4a, branch `fix/load-failed-row-viewer`, 2026-07-11) — a transport failure (`ResponseState::Failed { error, meta }`, produced by the main pane, load runner, AND sequence runner) now renders an **honest failure panel** and is **copyable with `y`**. Two confirmed defects fixed: (1) `y` was a silent no-op on a `Failed` row because `response_copy_view` went only through `response_view_mut()` (which is `Some` only for `Done`); (2) the panel showed only the raw error string. The shared `response::render` `Failed` arm now shows the request method+URL (from `meta`) + the error, plus an explicit line that status/body/timing are unavailable — a **transport failure genuinely has no HTTP response**, so nothing is fabricated (all three failure-construction sites already set the row `timing = None`). `y` copies the error message + request line via the same clipboard path (`enqueue_clipboard`). The fix lives entirely in the shared unified viewer surface — a new `ResponseState::failure_copy_text()` seam + one state-branch in `response_copy_view` — so all three runners get it at once and it cannot drift per-site. `Dropped` (load-runner memory-evicted rows) is deliberately left a copy no-op: its body bytes are gone and not reconstructable, so there is nothing honest to copy; its existing status/timing/size placeholder is unchanged. Tests: `main_failed_response_y_copies_error_and_request`, `load_failed_row_y_copies_error_not_noop`, `sequence_failed_step_y_copies_error_not_noop`, `failure_copy_text_shape`, plus the regenerated `response_pane_failed` / `sequence_runner_finished_with_failure` snapshots.

**Drive-test follow-up (note #6) — delete a sequence from the Sequences pane** (owner drive-test finding #6, branch `feat/n6-delete-sequence`, 2026-07-11) — `d` on the selected sequence in the Sequences sub-pane now opens a y/n confirm ("Delete sequence" / "Delete this sequence?") and, on `y`, deletes the sequence file and refreshes the pane; before, `d` there notified "not available in the sequences pane". The whole path **mirrors the existing endpoint-delete pattern exactly**: new `Action::DeleteSequence` + `ConfirmPurpose::DeleteSequence` + `confirm_text` entry + a `handle_confirm_key` branch, calling the already-present `persistence::delete_sequence(path)` (the twin of `persistence::delete_endpoint`, `std::fs::remove_file` → `PersistenceError::Write`). Key routing: the sequences-pane `Action::Delete` guard now dispatches `begin_delete_sequence()` (parallel to how `n`→new-sequence and `r`→run-sequence are already routed there); every other sequences-pane key is unchanged. Selection lands sensibly because `reload_explorer` clamps the sub-pane cursor to the new count (→ previous/next surviving sequence) and, when the list empties, `enforce_left_active_invariant` reconciles the left-column focus back to the endpoints tree (no stranded empty pane). A y/n confirm (not typed-name friction) matches the endpoint arm — a sequence file carries endpoint refs + extraction expressions, never secret values. The loaded/running case: the sequence editor/runner are modal (`Mode::Sequence`) so cannot be open while this confirm runs in `Mode::Normal`; defensively, if a stale `sequence_editor`/`sequence_runner` state still points at the just-deleted file it is dropped (so a later editor save can't resurrect the file and a runner can't act on a vanished target). Tests: `delete_sequence_removes_file` (persistence), `d_confirm_y_deletes_hovered_sequence`, `deleting_last_sequence_empties_pane_and_reconciles_focus`, `d_on_empty_sequences_subpane_notifies` (app-level), plus the updated `tree_mutating_keys_suppressed_on_sequences_subpane` assertion (`d` now confirms a delete instead of no-op).
**Drive-test follow-up (note #5) — refined tab chips + `<leader>t <n>` numbered jump** (owner drive-test finding #5, owner decisions LOCKED; branch `feat/n5-tabs`, 2026-07-11). Two parts, render/theme + keymap only. **Part A — softer chips (owner: "keep chips, refine")**: both tab bars (the top buffer strip `tab_strip.rs` and the request pane's Params/Headers/Auth/Body bar `request.rs`, sharing the `chip_window` scroll + `chip_width`/`CHIP_OVERHEAD` accounting) drop the hard `▐`/`▌` half-block edge caps from note #7 — the chip is now the padded fill itself: two-space internal padding each side (was one) for a roomier, softer cap, and a single raw space **gap** between chips does the separating (no cap glyph, no `▏` bar). `CHIP_OVERHEAD` drops 3→1 (just the trailing gap), while the wider label padding keeps chip widths ~unchanged so the keep-active-visible window, `‹`/`›` overflow markers, dirty `●`, and exactly-full right-reserve all behave as before. The active chip is now the bright `selection` fill **bolded** (`add_modifier(BOLD)`) for stronger active-vs-inactive contrast — applied to the local render style only, leaving the shared `selection` slot (explorer/picker rows) untouched; inactive stays the dim `tab_inactive` fill. **Part B — numbered jump**: a new parameterized `Action::FocusBufferIndex(usize)` (1-based) wired into the tabs leader submenu as `<leader>t 1`…`<leader>t 9`, jumping straight to the Nth open tab by reusing the same `self.active` focus mechanism `BufferNext`/`BufferPrev` drive (`App::focus_buffer_index`, no duplicated focus logic). Out-of-range (`n` > open count, and defensively `n == 0`) is a graceful no-op with a `no tab N` status message — never a panic or wrong-tab jump. The digit binds live ONLY in the `tabs` submenu keymap layer, so they never shadow the Request-pane `1`..`4` tab-jump overlay (`PaneCtx::Request`, M6.7) — two independent layers. The nine variants join `Action::all()`/`name()`/`label()`/`FromStr` via a small `FOCUS_BUFFER_TABLE` (config names `focus-buffer-1`..`9`), so they surface in the command palette, `churl keymaps`, the `?` help overlay, and the `<leader>t` which-key popup automatically. Tests: `refined_chips_drop_caps_and_bar_separators` (no `▐`/`▌`/`▏`, gap present, bold-active fill), updated `chips_carry_active_and_inactive_backgrounds` + `tab_label_widths`/`chip_width_counts_padding_and_gap`, `focus_buffer_index_focuses_nth_tab`, `focus_buffer_index_out_of_range_is_noop_with_message`, `numbered_jump_does_not_shadow_request_pane_digits`, extended `leader_tabs_submenu_binds`, plus regenerated tab-strip / request-tab-bar snapshots. See the 2026-07-11 note-#5 ADR in DECISIONS.md.
**Drive-test follow-up (note #3) — ephemeral peek + copy for masked secret values** (owner drive-test finding #3, branch `feat/n3-secret-peek`, 2026-07-11) — the owner needs to *reveal* a masked value to debug (confirm a token is correct), so the env & vars editor now has an **ephemeral peek**. In the var-rows pane, **`p`** on a **masked** row (a `SESSION` capture — always masked — or a secret-named non-placeholder literal) reveals its **resolved** value in place (run through the *same* `Resolver` a standalone send uses: `session > cli > profile > workspace > env`; the endpoint-specific `collection` scope is omitted since the editor is not tied to a loaded endpoint). While revealed, **`y`** copies that revealed value to the clipboard via the existing `enqueue_clipboard` path (the "allow copy" the owner asked for). The peek is **ephemeral** and re-masks automatically on **any of**: moving to another row, changing pane/mode, or a **6 s timeout** (`REVEAL_EXPIRE_SECS`, checked on the existing 250 ms tick — the same mechanism that expires transient messages). **Only one row** is ever revealed, pinned to a `(scope, row)` coordinate so a stale reveal can never paint another row's secret. A `👁 revealed · y copy · any key re-masks` affordance marks the visible row. **Security invariants**: the revealed plaintext lives **only** in the editor's transient `reveal: Option<Reveal>` view field — never written to disk, never persisted to any manifest/config/state file, never logged, never placed in an error/panic message; re-masking drops the `String`. The `SESSION` group stays **read-only under reveal** (peek never enters edit mode; add/delete/edit keys are still swallowed with the read-only message). The masking predicate (`row_is_masked`) is the *same* one `render_var_line` uses, so reveal and mask can never disagree. The editor holds no `Resolver` — it emits `EnvKeyOutcome::RevealRow`/`CopyRevealed` and the app resolves/copies (`build_env_resolver`), so the UI-free/core boundary is preserved. **Auth values in the request pane were NOT in scope** — that masking is a separate seam (`components/request.rs` renders `*****` directly, no shared reveal state), so this change is scoped to the env editor per the task's fallback. Tests (16 new): unit — reveal shows the handed-in resolved value + affordance, re-mask on row-move/mode-change/timeout, only-one-row-revealed, `p` no-op on a visible row, `y` emits `CopyRevealed` and keeps the reveal alive for read-back, `y` no-op without a reveal, `SESSION` stays read-only under reveal, reveal lives only in view state (rows/snapshot untouched, clear drops the plaintext); app-level — `p` reveals the resolved literal + `y` queues the copy, a peek+copy+close writes **nothing** to `churl.toml` (byte-identical), re-mask on row move. One env-editor footer snapshot regenerated (adds `p peek`).
**Drive-test follow-up (D-round notes #1/#2 + no-op fold-in) — session-marker contrast · env `g`/`G` nav · no silent copy no-ops** (owner drive-test 2026-07-11, branch `fix/n1-n2-ux`) — three small related UX fixes:
- **#1 — `→session` marker legible on the highlighted row.** The dim ` →session` marker on a Session-target extraction rule (note #6) used `theme.statusline`, which washed out against the selection background when its row was selected. A new **theme slot `session_marker`** (dark: cyan · light: blue; wired like `tab_inactive`) carries the plain-row style; the render (`session_marker_style` in `components/sequence_editor.rs`) derives a **selection-aware variant** — carrying `selection`'s background + a `DIM` modifier — so the marker stays legible *and* visually subordinate on both rows. Overridable via `[theme_colors] session_marker = …`.
- **#2 — env editor `g`/`G` navigation.** The env & vars editor (`components/env_editor.rs`) scope list and var-row list gained vim `g` (jump to first) / `G` (jump to last), aliased to Home/End, matching the **single-`g`** convention already used by the sequence-/load-runner lists (no `gg` chord). `g`/`G` are pure navigation, so they work in the read-only Session group too.
- **No-op fold-in.** `y` (`response_copy_view`) on a `Dropped` (memory-evicted) load row, and `Y` (`response_copy_line`) on any bodyless row (`Failed`/`Dropped`/…), were still silent no-ops post-note-#4a — the exact class of bug #4a was about. They now emit a status message via `self.message` (`nothing_to_copy_message`): "nothing to copy — response body not retained" for `Dropped`, "nothing to copy" otherwise. The `Done`/`Failed`-with-error copy paths from #4a are unchanged.

Tests: `session_marker_style_stays_legible_on_selected_row`, `g_and_shift_g_jump_scope_list_top_and_bottom`, `g_and_shift_g_jump_var_rows_top_and_bottom`, `load_dropped_row_y_sets_message_not_silent_noop`, `load_dropped_row_shift_y_sets_message_not_silent_noop`, `load_failed_row_shift_y_sets_message_not_silent_noop`. See the 2026-07-11 D-round ADR in DECISIONS.md.
